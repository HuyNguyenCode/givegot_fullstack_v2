...
| R-015 | Historical Task 00 lint baseline: 78 errors and 32 warnings, including scripts and application code. Current observed E1-era result: 151 errors and 34 warnings. The repository memory does not yet classify the delta or attribute its cause. | Medium | High | Preserve both measurements; classify the delta with evidence before treating it as pre-existing or task-introduced. Do not exclude or weaken relevant files. | Open: delta classification pending |
