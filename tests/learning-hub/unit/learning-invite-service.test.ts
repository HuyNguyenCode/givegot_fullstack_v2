...
import { createLearningInviteService, LearningInviteConflictError, type LearningInviteTransaction } from '../../../src/lib/learning-invite-service'
...
type InviteRecord = Pick<LearningInvite, 'id' | 'tokenHash' | 'inviterId' | 'acceptedById' | 'learningSpaceId' | 'primarySkillId' | 'objective' | 'status' | 'maxUses' | 'useCount' | 'expiresAt' | 'acceptedAt'> & { primarySkill?: { name: string } | null }
type SpaceRecord = Pick<LearningSpace, 'id' | 'state' | 'primarySkillId'> & { objective?: string | null }
type MemberRecord = Pick<LearningSpaceMember, 'learningSpaceId' | 'userId' | 'status'>
type SpaceCreateData = Pick<Prisma.LearningSpaceUncheckedCreateInput, 'title' | 'primarySkillId'> & Partial<Pick<Prisma.LearningSpaceUncheckedCreateInput, 'objective' | 'createdById' | 'updatedById'>>

function repository() {
  const invites = new Map<string, InviteRecord>(); const invitesByHash = new Map<string, InviteRecord>(); const spaces = new Map<string, SpaceRecord>(); const members = new Map<string, MemberRecord>(); let next = 1; let tail = Promise.resolve()
  const memberKey = (spaceId: string, userId: string) => `${spaceId}:${userId}`
  const transaction = async <T>(fn: (tx: LearningInviteTransaction) => Promise<T>) => {
  ...
  const tx = {
    ...
    createInvite: async (data: Prisma.LearningInviteUncheckedCreateInput) => { const invite = { id: `invite-${next++}`, learningSpaceId: null, acceptedById: null, status: 'ACTIVE' as const, useCount: 0, acceptedAt: null, revokedAt: null, createdAt: CREATED_AT, updatedAt: CREATED_AT, ...data } as InviteRecord; invites.set(invite.id, invite); invitesByHash.set(invite.tokenHash, invite); return invite },
    ...
    createSpace: async (data: SpaceCreateData) => { const space = { id: `space-${next++}`, state: 'ACTIVE' as const, ...data }; spaces.set(space.id, space); return space },
    createMember: async (learningSpaceId: string, userId: string) => { const member = { learningSpaceId, userId, status: 'ACTIVE' as const }; members.set(memberKey(learningSpaceId, userId), member); return member },
...
test('C1 creates a high-entropy hashed invite and accepts it exactly once for its pair', async () => {
  ...
  assert.equal(created.token, TOKEN); assert.equal((created.invite as typeof created.invite & { tokenHash?: string }).tokenHash, undefined); assert.notEqual([...repo.invites.values()][0].tokenHash, TOKEN)
...
test('C1 rejects expired, revoked, and self invites', async () => {
  ...
  await assert.rejects(service(expiredRepo, 'b', new Date('2026-09-16T00:02:00.000Z')).accept(expired.token), (error: unknown) => error instanceof LearningInviteConflictError && /expired/.test(error.message))
  const revokedRepo = repository(); const revoked = await create(revokedRepo); await service(revokedRepo, 'a').revoke(revoked.invite.id)
  await assert.rejects(service(revokedRepo, 'b').accept(revoked.token), (error: unknown) => error instanceof LearningInviteConflictError && /revoked/.test(error.message))
  const selfRepo = repository(); const self = await create(selfRepo)
  await assert.rejects(service(selfRepo, 'a').accept(self.token), (error: unknown) => error instanceof LearningInviteConflictError && /own/.test(error.message))
})
...
test('C1 replays idempotently for the accepting user and rejects reuse by another user', async () => {
  ...
  await assert.rejects(service(repo, 'c').accept(created.token), (error: unknown) => error instanceof AuthorizationError && error.status === 403)
})
...
test('C1 rejects reuse that would create a third active member', async () => {
  ...
  await assert.rejects(service(repo, 'b').accept(created.token, { reuseSpaceId: existing.id }), (error: unknown) => error instanceof LearningInviteConflictError && /third/.test(error.message))
})
