...
const body = async (request: NextRequest) => { const value: unknown = await request.json(); if (!value || typeof value !== 'object' || Array.isArray(value)) throw new Error('Invalid body'); return value as any }
...
export function createLearningInvitePreviewHandlers(service: Service, continuation = setLearningInviteContinuation) { return { POST: async (request: NextRequest) => { try { const input = await body(request); const preview = await service.preview(input.token); const response = NextResponse.json({ preview, authenticationRequired: true }); continuation(response, input.token); return response } catch (error) { return fail(error) } } } }
export function createLearningInviteAcceptHandlers(service: Service, readContinuation = readLearningInviteContinuation) { return { POST: async (request: NextRequest) => { try { const input = await body(request); const token = input.token ?? readContinuation(request); if (!token) throw new LearningInviteValidationError('An invite token or valid login continuation is required'); const accepted = await service.accept(token, input); const response = NextResponse.json(accepted); clearLearningInviteContinuation(response); return response } catch (error) { return fail(error) } } } }
export function createLearningInviteHandlers(service: Service) { return { PATCH: async (request: NextRequest, context: any) => { try { return NextResponse.json({ invite: await service.revoke((await context.params).inviteId) }) } catch (error) { return fail(error) } } } }
