...
export default function ProfilePage() {
  ...
  const [showCustomSkillToast, setShowCustomSkillToast] = useState(false)
  ...
  // Show a temporary toast and auto-dismiss after `ms` milliseconds
  const triggerCustomSkillToast = () => {
    setShowCustomSkillToast(true)
    setTimeout(() => setShowCustomSkillToast(false), 6000)
  }
  ...
  const addTeachingSkill = (skillName: string) => {
    ...
    if (trimmed && !selectedTeachingSkills.includes(trimmed)) {
      // Detect if this is a brand-new custom skill (not in the existing DB list)
      const isCustom = !availableSkills.some(
        s => s.name.toLowerCase() === trimmed.toLowerCase()
      )
      if (isCustom) triggerCustomSkillToast()
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-purple-50 via-white to-blue-50">
      ...
      {showCustomSkillToast && (
        <div className="fixed bottom-8 left-1/2 -translate-x-1/2 max-w-md w-full bg-amber-500 text-white px-6 py-4 rounded-lg shadow-2xl flex items-start gap-3 animate-slide-in z-50">
          ...
          <p className="text-sm leading-relaxed">
            Kỹ năng mới đã được ghi nhận và đang chờ Admin duyệt để có thể ghép đôi. Để tìm kiếm nhanh nhất, bạn nên ưu tiên chọn các kỹ năng có sẵn trong danh sách!
          </p>
          <button
            type="button"
            onClick={() => setShowCustomSkillToast(false)}
